...
Both the M5 stack firmware and encoder module firmware are in the same repository. You can find them here:
...
[KER M5 Stack firmware](https://github.com/enactic/openarm_ker_firmware/tree/main/M5)
...
```bash
https://github.com/enactic/openarm_ker_firmware
cd openarm_ker_firmware/encoder
uv venv 
source .venv/bin/activate
uv pip install platformio
```
...
<BlockImage src="apiref/setup_ker/ker-encoder-write.JPG" alt="encoder connection" width="90%" />

Then run the following command to flash the firmware.

Following command is the example  flashes ID=3. Change DEVICE_ID to flash a different ID.
